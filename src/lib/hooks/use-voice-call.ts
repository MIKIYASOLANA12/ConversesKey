...
export function useVoiceCall(personalityId: PersonalityId, emotion: Emotion, scenarioId?: ScenarioId) {
  ...
  // Use a ref to allow recursive retry without circular useCallback dependency
  const getAIResponseRef = useRef<((text: string) => Promise<void>) | null>(null);
  ...
  const getAIResponse = useCallback(async (userText: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const errorMessage = errorBody?.error || `API returned ${response.status}`;

        if (response.status === 429) {
          const retryAfter = errorBody?.retryAfterSeconds ?? 60;
          console.warn(`[useVoiceCall] Quota exceeded. Retrying in ${retryAfter}s...`);
          setError(`AI service is busy. Retrying in ${retryAfter} seconds...`);

          // Wait for the retry delay, then retry via the ref
          await new Promise((resolve) => setTimeout(resolve, retryAfter * 1000));
          getAIResponseRef.current?.(userText);
          return;
        }

        console.error(`[useVoiceCall] API error ${response.status}:`, errorMessage);
  ...
  // Store the callback in the ref for recursive retry
  useEffect(() => {
  ...
  const startCall = useCallback(async () => {
    ...
    if (callStateRef.current === 'connecting') {
      getAIResponse(`[System Note: The user just joined the call. You must speak first. Greet the user by stating your name, expressing your current emotion (${emotion}), and introducing the topic of conversation. Keep it concise.]`);
    } else {
